...
- **Keyboard navigation in airport search results** — Currently the zone-search dropdown (`#zone-search-results`) renders clickable results but keyboard navigation is limited. Improve UX for keyboard-only users or faster keyboard-driven search workflows. Acceptance criteria: (1) arrow keys (↑↓) navigate through results (wrap at ends), (2) first result is highlighted/focused by default when results appear (not just visually focused but also ready for activation), (3) **Enter** selects the currently-highlighted result and triggers the zone change (same behavior as a click), (4) **Escape** closes the dropdown, (5) highlighting persists and updates in real-time as user types a new query. Visual feedback: highlighted result has `.active` class applied (already used for basemap/category dropdowns, so styling exists); arrow navigation updates the highlighted item immediately. Implementation: in `static/js/state-filters.js`'s zone-search handler, add a `keydown` listener on `#zone-search-input` that dispatches on key code (ArrowUp/ArrowDown/Enter/Escape), maintains a `currentHighlightedIndex` variable, and applies/removes `.active` class on result elements; on Enter, programmatically trigger the selection behavior (same as `e.target.click()` on the highlighted `.dropdown-option`). Estimate: XS–S (keyboard event plumbing + class toggling, ~1–2h).
