...
- **Speed/value estimate required (since 2026-07-21):** every item added or materially edited must get a row in the "At a glance" table at the top of `.ai/BACKLOG.md` — Speed (`XS`/`S`/`M`/`L`/`XL`) and Value (`Low`/`Medium`/`High`), judged against the app's actual current use, not hypothetical scale. This is what the table is for; don't let it drift out of sync with the item sections below it.
