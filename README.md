<img width="1167" height="912" alt="Screenshot 2026-07-28 at 18 01 52" src="https://github.com/user-attachments/assets/cee710e3-44f3-4822-93d0-0e02cd679e56" />
...
confirmed live from a Northflank pod — DNS resolves fine and unrelated
hosts connect in well under a second, but a raw TCP connect to
`opensky-network.org` timed out on both port 80 and 443, meaning the
connection was being dropped at OpenSky's end, not failing locally. The
app degrades gracefully (see `app.py`'s outage circuit-breaker) rather than
this taking the rest of the app down with it, so the OpenSky source simply
goes empty rather than breaking anything else when this happens. That said,
the block is evidently intermittent, not permanent: the same deployment was
confirmed reachable again a few hours after being diagnosed as blocked, on
the same day — so treat "OpenSky is down" and "OpenSky is up" as both
possible on a hyperscaler deployment at any given time, not a fixed
one-or-the-other verdict. It's unaffected running locally or on non-
